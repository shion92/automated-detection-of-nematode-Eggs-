...
def cross_validate(num_folds=5, num_epochs=20, batch_size=2):
    set_seed()
    # Build datasets once
    train_images = os.path.join(TRAIN_DIR, "images")
    train_anns   = os.path.join(TRAIN_DIR, "annotations")

    ds_aug  = VOCDataset(train_images, train_anns, transforms=get_train_transforms())
    ds_plain= VOCDataset(train_images, train_anns, transforms=F.to_tensor)

    kf = KFold(n_splits=num_folds, shuffle=True, random_state=42)
    fold_losses = []

    for fold, (train_idx, val_idx) in enumerate(kf.split(ds_aug), start=1):
        print(f"\n=== Fold {fold}/{num_folds} ===")
        train_sub = Subset(ds_aug,   train_idx)
        val_sub   = Subset(ds_plain, val_idx)

        train_loader = DataLoader(train_sub, batch_size=batch_size, shuffle=True,  collate_fn=_collate_fn)
        val_loader   = DataLoader(val_sub,   batch_size=batch_size, shuffle=False, collate_fn=_collate_fn)

        # fresh model per fold
        weights = FasterRCNN_ResNet50_FPN_Weights.DEFAULT
        model = fasterrcnn_resnet50_fpn(weights=weights)
        in_f = model.roi_heads.box_predictor.cls_score.in_features
        model.roi_heads.box_predictor = torchvision.models.detection.faster_rcnn.FastRCNNPredictor(in_f, NUM_CLASSES)
        model.to(DEVICE)
        ...
        for epoch in range(num_epochs):
            ...
            with torch.no_grad():
                for imgs, targets, _ in val_loader:
                    ...
                    v_loss += sum(ld.values()).item()
            avg_val = v_loss / len(val_loader)
            if avg_val < best_val:
                best_val = avg_val
            print(f"Fold {fold} E{epoch+1}: val_loss={avg_val:.4f}")
        print(f"-> Fold {fold} best_val_loss = {best_val:.4f}")
        fold_losses.append(best_val)

    mean_loss = sum(fold_losses)/len(fold_losses)
    print(f"\n=== CV complete: fold_losses={fold_losses}, mean={mean_loss:.4f} ===")
    ...
    print("Saved cross-validation results to cv_results.json")
...
if __name__ == "__main__":
    ...
    cross_validate(num_folds=5, num_epochs=num_epochs, batch_size=2)
    total = time.time() - start
    print(f"✅ CV runtime: {total:.1f}s")